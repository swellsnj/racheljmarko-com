/*
  ...
  CLICKING A TILE: instead of linking out to the file's Google Drive page (where a visitor
  could hit Drive's own "Download" button and grab the original, full-resolution file), each
  tile opens an on-site preview drawn onto a <canvas> with "© Rachel J. Marko" watermarked
  across it. Two things make this a real (if not bulletproof) deterrent: the preview only ever
  requests Drive's *thumbnail* image, capped around 1400px — nowhere near Rachel's original
  file resolution — and the watermark is baked into the rendered pixels, not just an overlay,
  so a right-click "save image" grabs the watermarked, capped version, not a clean file.
  It won't stop a determined screenshotter, and nothing client-side can. For real protection
  Rachel can also turn off "viewers can download, print, and copy" in each folder's Drive
...
async function loadGallery() {
  ...
  function renderGrid(files) {
    grid.innerHTML = files.map((f, i) => `
      <button class="tile" type="button" data-index="${i}">
...
/* ---- watermarked lightbox preview ---- */

function upsizeThumbnail(thumbnailLink, size) {
...
function drawWatermark(ctx, w, h) {
  ...
  ctx.lineWidth = 1;
  const fontSize = Math.max(16, Math.round(w / 24));
  ctx.font = `600 ${fontSize}px sans-serif`;
  ctx.textBaseline = 'middle';
  const text = '© Rachel J. Marko';
...
function openLightbox(file) {
  ...
  img.onerror = () => {
    // Fall back to a plain capped-resolution image if the canvas draw fails for any reason
    // (still nowhere near full res, just without the baked-in watermark).
    canvas.hidden = true;
    fallbackImg.hidden = false;
    fallbackImg.src = upsizeThumbnail(file.thumbnailLink, 1400);
  };
